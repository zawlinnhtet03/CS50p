...
#include <conio.h> // Include conio.h on Windows
#include <fstream>
...
class snake //organizing variables and functions by class named "snake"
{
    public:
        ...
        int boardWidth; // Dimensions of the game board
        int boardHeight;

        int score;

        int foodx, foody; // Current coordinates of the food

        // Handle the console window and store the current cursor position
...
int d = 4; // Glsgal variable for direction more of which in insert function
...
snake::snake() //setting NULL values, setting coordinates for food and wall
{
    ...
    current = NULL;
    newtail = NULL;
    console_handle = GetStdHandle(STD_OUTPUT_HANDLE); // Console manupulation

    // Fixed original positiion for the food
    foodx = 12;
    foody = 14;
...
void snake::gameOver() {
    ...
    SetConsoleCursorPosition(console_handle, cur_cord);
    cout << "Thank you for enjoying the game" <<endl;
...
void loop(snake &sg) {
    ...
    } while (chosenLev != 1 && chosenLev != 2 && chosenLev != 3);
}
